...
class ExactMatchStage
{
    ...
    {
        // 1. Exact match for complete normalized query
        ...
        // 2. Also check for exact matches of individual words
            ...
            if (isset($context->wordIndex[$queryWord])) {
                foreach ($context->wordIndex[$queryWord] as $match) {
                    ...
                    if (!isset($context->seen[$resultKey])) {
                        ...
                        if ($model) {
                            $score = 0.95 * $match['weight']; // Légèrement moins que la phrase complète
